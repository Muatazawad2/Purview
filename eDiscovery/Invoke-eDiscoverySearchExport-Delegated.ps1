<#
.SYNOPSIS
    Purview eDiscovery Search & Export Tool (Delegated user authentication)
    
.DESCRIPTION
    Interactive menu-driven PowerShell application for exploring Microsoft Purview eDiscovery cases.
    Connects using delegated user authentication (browser-based sign-in) via Microsoft Graph API.
    
    Features:
    - Discover all available eDiscovery cases in the tenant
    - View detailed case information on demand
    - Explore searches with item counts and sizes
    - Review hold policies and custodians
    - List export operations and download case files
    - Real-time dashboard showing case statistics
    
    Authentication: User signs in via browser (OAuth 2.0 interactive flow).
    Permissions: Requires eDiscovery.Read.All scope or equivalent permissions.
...
# AUTHENTICATION
# ============================================================================
# Connect to Microsoft Graph with delegated user authentication (browser sign-in)
Show-PanelTitle -Title "Purview eDiscovery Search & Export" -Subtitle "Delegated user workspace session"
Write-Host "[*] Opening browser for interactive sign-in..." -ForegroundColor Cyan
try {
    Connect-MgGraph -Scopes "eDiscovery.Read.All", "offline_access" -NoWelcome -ErrorAction Stop
    Write-Host "[OK] Delegated authentication established" -ForegroundColor Green
}
catch {
    Write-Error "Failed to connect with delegated authentication: $_"
    return
}
